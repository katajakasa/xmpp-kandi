...
Pikaviestinn‰st‰ on tullut osa useiden tietokoneen, ja nyky‰‰n myˆs erilaisten mobiililaitteiden 
k‰ytt‰jien arkea. Pikaviestint‰‰ k‰ytet‰‰n niin kotona, kuin myˆs koulussa~\cite{im_at_school} ja tˆiss‰~\cite{im_at_work}. 
Erilaisia pikaviestinsovelluksia on useita, ja viestint‰‰ voidaan harrastaa joko perinteisesti tekstiviestein, tai 
monipuolisemmin ‰‰nen ja videon avulla. WWW-ohjelmointitekniikoiden edistyess‰ pikaviestimi‰ lˆytyy jopa erilaisilta 
...
n‰iden sovellusten k‰yttˆˆn. T‰st‰ johtuukin nykymallin mukaisen pikaviestinn‰n suurin ongelma; eri pikaviestinverkkojen k‰yttˆˆn
tarvitaan usein eri sovellukset. Samalla k‰ytt‰j‰ll‰ saattaakin olla k‰ytˆss‰ useita ohjelmia, jotka toimivat omissa
...
yleisk‰yttˆinen protokolla kaikkeen pikaviestint‰‰n~\cite{about_xmpp}. Se hallitsee laajennuksia k‰ytt‰en mm. tekstiviestit, 
...
suosituimpia sovelluksia nyky‰‰n ovat muunmuassa facebook, MSN Messenger, Skype sek‰ puhelimilla tapahtuva tekstiviestint‰. Myˆs muita
ohjelmia k‰ytet‰‰n.
...
sek‰ vanhempien v‰liseen viestint‰‰n. Myˆs kursseja tai tunteja saatetaan j‰rjest‰‰ et‰n‰ k‰ytt‰en apuna ‰‰ni- video- ja tekstipohjaista 
pikaviestint‰‰.

\subsection{Muita protokollia}

T‰ss‰ tutkielmassa tarkasteltavan XMPP-protokollan lis‰ksi on olemassa myˆs muita pikaviestinprotokollia, kuten muunmuassa 
\emph{MSNP}, \emph{Skype}, \emph{IRC}, \emph{SIMPLE} ja useita muita v‰hemm‰n k‰ytettyj‰. N‰ist‰ SIMPLE suunniteltiin myˆs 
mahdollisimman yleisk‰yttˆiseksi ja avoimeksi, ja on k‰ytˆss‰ esimerkiksi joissain VoIP-sovelluksissa. 
...
tilan (Presence status) hallintaan. Kehitystyˆn tuloksena oli lopulta dokumentti, jossa m‰‰riteltiin protokollan perusominaisuudet 
ja laajennukset pikaviestint‰‰n. Nyky‰‰n t‰t‰ perustoiminnallisuutta m‰‰ritell‰‰n RFC-do\-ku\-men\-teis\-sa 3920 ja 3921. 
Myˆhemmin protokollaan on luotu laajennuksia esimerkiksi tiedostojen siirt‰miseen sek‰ VoIP-puheluihin. 
...
voivat myˆhemmin p‰‰st‰ mukaan itse protokollan ydinm‰‰ritelm‰‰n. Suurin osa laajennuksista on t‰ll‰ hetkell‰ kuitenkin
viel‰ suunnitelma- tai testausasteella, ja onpa olemassa kaksi virallista, l‰hinn‰ huumorin kannalta kehitetty‰ 
laajennustakin. Asiakasohjelmien ei teoriassa ole pakko tukea kaikkia laajennuksia, mutta k‰yt‰nnˆss‰ asiakasohjelmat
...
\begin{itemize}
  ...
  \item XEP-0095: Stream Initiation. Laajentaa xml-streamien k‰yttˆtapoja, jotta niiss‰ voidaan siirt‰‰
        mm. tiedostoja.
...
Virran alku ilmoitetaan aina XML-elementill‰ \emph{stream}, jossa voidaan m‰‰ritell‰ attribuutteina myˆs esimerkiksi k‰ytett‰v‰ 
nimiavaruus ja nerkistˆ. Niin kauan kuin XML-virta on olemassa, sen luonut entiteetti voi l‰hett‰‰ m‰‰ritt‰m‰ttˆm‰n 
...
Asiakas-palvelin yhteyksien lis‰ksi palvelin-palvelin yhteydet ovat myˆs mahdollisia.

\subsection{S‰keistˆt}

XMPP-protokollassa s‰keistˆ (Stanza) on xml-elementti sis‰ltˆineen, joka voidaan l‰hett‰‰ entiteetilt‰ toiselle XML-virran yli. 
S‰keistˆ kuljetetaan aina suoraan XML-virran \emph{stream}-juurielementin alla, ja voi olla nimelt‰‰n \emph{message}, \emph{presence} tai \emph{iq}.
S‰keistˆjen nimiavaruuden on oltava \emph{jabber:client} tai \emph{jabber:server}. Mik‰‰n muu tietovirrassa l‰hetetty elementti
ei ole s‰keistˆ. S‰keistˆ sis‰lt‰‰ yleens‰ yhden tai useampia lapsielementtej‰.

S‰keistˆll‰ on kolme tyyppi‰, joilla m‰‰ritet‰‰n viestint‰mekanismi. \emph{Push}-mekanismi on tarkoitettu yleiseen viestint‰‰n,
...
vaihtamiseen entiteettien kesken. \emph{Push}-mekanismissa kyse on siit‰, ett‰ entiteetti tyˆnt‰‰ toiselle entiteetille tietoa,
...
ei ole yht‰ keskitetty‰ autentikaatiopalvelinta. K‰yt‰nnˆss‰ siis kuka tahansa voi ajaa omaa 
...
Authority (IANA) on m‰‰ritellyt XMPP-protokollan k‰yttˆˆn portin 5222, mutta muitakin voi olla k‰ytˆss‰. 
...
Entiteeteille voidaan m‰‰ritt‰‰ myˆs t‰rkeystasoja, jolloin esimerkiksi osoitteeseen \emph{solmu@toimialue/matkapuhelin} 
...
XMPP-verkossa m‰‰ritell‰‰n myˆs palvelinten v‰linen takaisinsoitto (Server dialback), jota voidaan k‰ytt‰‰
...
palvelin voi l‰hett‰‰ myˆs \emph{failure}-elementin virhetietojen kera. L‰hetett‰v‰t elementit riippuvat valitusta 
...
kaikkiin resursseihin. Elementiss‰ voidaan m‰‰ritell‰ myˆs \emph{from}-parametri, johon sijoitetaan viestin l‰hett‰j‰n JID.
...
mutta asiakasohjelmat saattavat itse lyhent‰‰ viesti‰. S‰e voi sis‰lt‰‰ myˆs \emph{thread}-elementin, jolla ilmoitetaan keskustelu,
...
sis‰llˆttˆm‰n presence-s‰keen, jonka \emph{type}-parametrina on \emph{subscribe} ja \emph{to}-parametrina lis‰yksen kohteen JID ilman resurssia.
...
Tilanmuutospyyntˆ tehd‰‰n l‰hett‰m‰ll‰ palvelimelle presence-s‰e. Tila ilmoitetaan s‰keess‰ elementiss‰ \emph{show}. Asiakas 
voi myˆs l‰hett‰‰ lis‰tietoja tilasta \emph{status}-elementiss‰. Lis‰tietoelementiss‰ voi olla pitempikin teksti, mutta tilaelementin
...
menetelm‰n haittana onkin huono hyˆtysuhde sek‰ mahdollisesti v‰liss‰ olevat palvelimet, jotka saattavat rajoittaa yhteyden nopeutta.
Kuvassa ~\ref{fig:siirtomenetelmat_cmp} kuvataan IBB-menetelm‰n ja SOCKS5-menetelm‰n siirretyn tiedon m‰‰r‰‰ k‰ytett‰ess‰ yhden 
megatavun hyˆtykuormaa. IBB-menetelm‰ sopii kuitenkin hyvin tapauksiin, joissa muut menetelm‰t eiv‰t toimi esimerkiksi palomuurien 
...
SI File Transfer-laajennus~\cite{xep0096} nimenomaan k‰ytt‰jien tiedostojen siirtoon tarkoitetun menetelm‰n. SI File Transfer hyˆdynt‰‰
p‰‰asiallisesti SOCKS5 Bytestreams-laajennusta, mutta voi myˆs tarvittaessa k‰ytt‰‰ IBB-siirtoja. Laajennus ei tuokaan lis‰‰
...
sen odotetaan tulevaisuudessa k‰yt‰nnˆss‰ korvaavan SI File Transfer-laajennuksen.
...
palveluista lˆytyy hyv‰n‰ esimerkkin‰ Facebook~\cite{fbchatapi}.

Suhteessa esimerkiksi MSNP- tai OSCAR-protokolliin, on XMPP helpompi ottaa k‰yttˆˆn omassa sovelluksessa. Sen koko dokumentaatio
on julkinen, ja kehitysprosessi on t‰ysin n‰kyv‰ kaikille asiasta kiinnostuneille. Suljetut, niin sanotut yhden yhtiˆn protokollat
kuten Skypen protokolla, MSNP ja OSCAR ovat vain osittain julkisesti dokumentoituja, ja kaikissa tapauksissa dokumentaatio
ei ole protokollan kehitt‰neen yhtiˆn julkaisemaa. Lis‰ksi ei ole mit‰‰n takeita siit‰, ett‰ protokollan m‰‰ritelm‰ pysyy samanlaisena
aina, eik‰ sen kehitykseen useinkaan voi vaikuttaa kehitt‰j‰yhtiˆn ulkopuolelta. Esimerkiksi MSNP- ja Skype-protokollat p‰ivittyv‰tkin
...
XML-pohjaisten protokollien heikkoutena on pakettien suuri koko ja suhteellisen pieni hyˆtysuhde. XMPP ei ole poikkeus.
...
Internet-yhteyksill‰. Esimerkiksi bin‰‰ritiedon siirtoon protokollasta lˆytyy hyv‰ll‰kin hyˆtysuhteella varustettuja menetelmi‰.
...
(Secure Sockets Layer). TLS onkin paranneltu versio SSL-protokollasta. Yleisimpi‰ k‰yttˆtarkoituksia
...
Pikaviestint‰ on nyky‰‰n arkea jo tyˆpaikoilla, kouluissa ja muussakin arkiel‰m‰ss‰. Erilaisten
sovellusten myˆt‰ vaihtoehtoja pikaviestint‰‰n on useita. Vaikka onkin hyv‰ ett‰ jokaiselle lˆytyy 
...
tuella eteenp‰in. Protokollan suurin hyˆty onkin sen avoin dokumentaatio, hyvin suunniteltu m‰‰ritelm‰, 
